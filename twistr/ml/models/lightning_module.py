...
from twistr.ml.features import build_features
from twistr.ml.features.chi_angles import chi_mask
from twistr.ml.features.residue_type import one_hot_residue_type
from twistr.ml.losses import (
...
class ExamplesModule(pl.LightningModule):
    ...
    def _annealed_coord_mse_weight(self, training: bool) -> float:
        """Train: linearly interpolate start → floor over `coord_mse_annealed_steps`.
        Val: pinned to `start_weight` so val totals stay comparable across the
        whole run."""
        if not training:
            return self.cfg.coord_mse_annealed_start_weight
        progress = min(self.global_step / max(self.cfg.coord_mse_annealed_steps, 1), 1.0)
        start = self.cfg.coord_mse_annealed_start_weight
        floor = self.cfg.coord_mse_annealed_floor_weight
    ...
    def _losses_from_out(
        ...
        loss_dunbrack = self.cfg.dunbrack_weight * dunbrack_rotamer_loss(
            out["torsion_sincos"],
            batch["residue_type"],
            batch["is_helix"],
            chi_mask(batch["residue_type"]),
            batch["padding_mask"],
        )
        ...
        # Annealed: every helix real residue (all 14 slots) ∪ every non-helix
        # interface-residue real residue (sidechain slots only).
        helix_inclusion = (is_helix & real).unsqueeze(-1)                            # (B, N, 1) → broadcasts to all 14
        ...
        return {
            "helix": loss_helix,
            "bce": loss_bce,
            "vdw": self.cfg.vdw_loss_weight * geom["vdw"],
            "hbond": self.cfg.hbond_loss_weight * geom["hbond"],
            "parallel_displaced": self.cfg.parallel_displaced_loss_weight * geom["parallel_displaced"],
            "sandwich": self.cfg.sandwich_loss_weight * geom["sandwich"],
            "t_shaped": self.cfg.t_shaped_loss_weight * geom["t_shaped"],
            "clash": loss_clash,
            "backbone_continuity": loss_backbone_continuity,
            "dunbrack": loss_dunbrack,
            "coord_mse_antigen_backbone": self.cfg.coord_mse_antigen_backbone_weight * coord_mse_antigen_backbone,
            "coord_mse_annealed": self._annealed_coord_mse_weight(training) * coord_mse_annealed,
        }
