...
The detector at `twistr/pipeline/features/interaction_matrix.py` defines the bands;
this module recomputes the underlying distances/cosines/centroids directly
(the detector returns sigmoid-banded scores, not raw out-of-range distances).
...
via the AF2 psi rigid group — see `twistr/pipeline/models/sidechain.py:apply_torsions_to_atom14`.
...
def interaction_geometry_losses(
    ...
    # group (see twistr/pipeline/models/sidechain.py); pass `coords_ang` directly
