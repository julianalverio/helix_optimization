...
`dev/tools/local/dunbrack/fit_rotamer_library.py` and persisted to
`twistr/pipeline/losses/_dunbrack_library.npz`.
...
# In-package copy is the source of truth at training time (runtime/data/ is
# excluded from the launcher's source upload). The runtime/data/ location
# remains a fallback for local dev runs.
_PACKAGE_LIBRARY = Path(__file__).with_name("_dunbrack_library.npz")
_DATA_LIBRARY = (
    Path(__file__).resolve().parents[2]
    / "runtime" / "data" / "dunbrack" / "_dunbrack_library.npz"
)
...
def _load_library(device: torch.device, dtype: torch.dtype):
    ...
    if not _LIBRARY_PATH.exists():
        raise RuntimeError(
            ...
            "Build it once with: "
            "uv run python dev/tools/local/dunbrack/fit_rotamer_library.py"
        )
