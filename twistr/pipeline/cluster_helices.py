...
Run: `python -m twistr.pipeline.cluster_helices`.
"""
...
DEFAULT_MANIFEST = Path("data/module3/module3_manifest.parquet")
DEFAULT_OUTPUT = Path("data/module3/helix_clusters.parquet")
...
def _build_candidate_graph(
    ...
    for start in range(0, n, chunk):
        ...
        if (start // chunk) % 10 == 0:
            print(
                f"    chunk {start // chunk + 1}/{(n + chunk - 1) // chunk}: "
                f"running pairs={sum(len(a) for a in rows_all):,}",
                flush=True,
            )
    if not rows_all:
...
def cluster_helices(
    ...
    n_uniq = len(seqs)
    print(
        f"  {n_uniq:,} unique helix_sequences "
        f"(lengths {lens.min()}..{lens.max()}, mean {lens.mean():.1f})"
    )

    tau_max = max(tau_grid)
    print(f"building candidate-pair graph (tau_max={tau_max:.2f})")
    t0 = time.perf_counter()
    coo = _build_candidate_graph(seqs, lens, tau_max)
    print(
        f"  {coo.nnz:,} candidate pairs in {time.perf_counter() - t0:.1f}s"
    )
    ...
    print(
        f"  {n_comp:,} connected components; size: "
        f"max={largest}, top10={np.sort(comp_sizes)[::-1][:10].tolist()}, "
        f"singletons={(comp_sizes == 1).sum():,}"
    )
    ...
    print(f"clustering each component at τ ∈ {[f'{t:.2f}' for t in tau_grid]}")
    ...
    for ti, tau in enumerate(tau_grid):
        ...
        if score > best_score:
            ...
            best_ti = ti
        print(
            f"  τ={tau:.2f}: n_clusters={n_clusters:,}, "
            f"silhouette={score:+.4f}"
        )
    chosen_tau = tau_grid[best_ti]
    print(
        f"chosen τ={chosen_tau:.2f} (silhouette={best_score:+.4f}, "
        f"{next_cid[best_ti] - 1:,} clusters)"
    )
    ...
    print(
        ...
        f"mean={sizes.mean():.1f}, p50={pct[0]:.0f}, p75={pct[1]:.0f}, "
        f"p90={pct[2]:.0f}, p99={pct[3]:.0f}, "
