...
class MLConfig:
    ...
    manifest_path: str = "runtime/data/examples/module3_manifest.parquet"
    examples_root: str = "runtime/data/examples"
    cluster_path: str = "runtime/data/examples/helix_clusters.parquet"
    val_count: int = 1000
    seed: int = 0
    ...
    # cfg at the start of training (see twistr/pipeline/training/batch_calibration.py).
    ...
    # Interaction-matrix conditioning noise. See twistr/pipeline/features/interaction_matrix.py
    ...
    # twistr/pipeline/losses/interaction_geometry.py. Down-weighted vs BCE because at init
    ...
    # Geometric-loss handoff: each <loss>_loss_weight above is the end weight;
    # <loss>_loss_start_weight is the step-0 weight. Training linearly
    # interpolates over `geometric_handoff_ramp_steps`, then holds at end.
    # Defaults (start == end, steps == 0) are no-op. Validation pins each
    # loss to max(start, end) so val totals stay comparable across the run.
    geometric_handoff_ramp_steps: int = 0
    vdw_loss_start_weight: float = 0.1
    hbond_loss_start_weight: float = 0.1
    parallel_displaced_loss_start_weight: float = 0.1
    sandwich_loss_start_weight: float = 0.1
    t_shaped_loss_start_weight: float = 0.1

    # Packing-neighbor loss. One-sided relu(n_target - count) penalty on
    # interface residues' packing atoms (aliphatic stubs ∪ aromatic ring
    # atoms — see twistr/pipeline/features/interaction_matrix.py:PACKING_ATOMS). Counts
    # heavy-atom neighbors in band [d_lo, d_hi] Å on residues with
    # supervised positions only (helix ∪ antigen backbone ∪ antigen interface
    # sidechain). See twistr/pipeline/losses/packing.py.
    packing_loss_weight: float = 0.1
    packing_loss_start_weight: float = 0.1   # no-op ramp by default
    packing_loss_n_target: float = 4.0
    packing_loss_d_lo: float = 3.3           # Å
    packing_loss_d_hi: float = 5.5           # Å
    packing_loss_tau: float = 0.3            # Å, sigmoid steepness
    ...
    # See twistr/pipeline/losses/backbone_continuity.py.
    ...
    # dev/tools/local/dunbrack/fit_rotamer_library.py: 'helix' applied to is_helix
    # residues, 'general' to the antigen partner. Per-cell vMM with K=3
    # components. See twistr/pipeline/losses/dunbrack.py.
