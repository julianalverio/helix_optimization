...
from twistr.pipeline.epitopes.masif_runner import run_masif_site
from twistr.pipeline.epitopes.patches import extract_patches, patches_to_rows
...
def run_batch(ctx: PipelineContext) -> None:
    if ctx.pdb_ids is None:
        raise ValueError(
            "MaSIF stage requires a PDB list; pass --pdb-list to pipeline-run"
        )
