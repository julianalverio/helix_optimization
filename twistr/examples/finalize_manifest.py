...
Usage:
    python -m twistr.examples.finalize_manifest
"""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runtime/data/examples"))
    ...
    logger.info("Scanning %s...", examples_root)
    npz_files = sorted(examples_root.rglob("*.npz"))
    logger.info("  %d example .npz files", len(npz_files))
    ...
    for i, p in enumerate(npz_files, 1):
        ...
        if i % 50000 == 0:
            logger.info("  scanned %d/%d (%ds)", i, len(npz_files), int(time.time() - t0))
    ...
    logger.info("Wrote %s (%d rows)", example_path, len(example_df))

    logger.info("Scanning %s...", markers_root)
    marker_files = sorted(markers_root.rglob("*.marker"))
    logger.info("  %d markers", len(marker_files))
    ...
    logger.info("Wrote %s (%d rows)", entry_path, len(entry_df))

    # User-facing summary — printed (not logged) since it's the tool's product output.
    print()
    print("=" * 60)
    print("Summary")
    print("=" * 60)
