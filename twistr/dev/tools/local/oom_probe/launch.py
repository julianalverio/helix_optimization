...
submodules), uploads via ssh, runs `python -m twistr.pipeline.training.probe`,
...
Prereqs:
  ...
  - <examples_root>/.lengths.json populated locally — run
      `python -m twistr.pipeline.training.probe --compute-lengths-only --config runtime/configs/ml.yaml`
    first if missing.
"""
...
DEFAULT_CONFIG = REPO_ROOT / "runtime/configs/ml.yaml"
...
DATA_KEEP = {
    "runtime/data/examples/module3_manifest.parquet",
    "runtime/data/examples/helix_clusters.parquet",
    "runtime/data/examples/.lengths.json",
}
...
# are loaded from Protenix in `twistr/pipeline/features/chi_angles.py`), so don't
...
def _check_prereqs(config_path: Path) -> Path:
    ...
    if not lengths_sidecar.exists():
        sys.exit(
            ...
            "  .venv/bin/python -m twistr.pipeline.training.probe "
...
def main() -> None:
    ...
    try:
        ...
        rc = _ssh_exec_streaming(
            host, port,
            "bash /workspace/twistr/dev/tools/local/oom_probe/bootstrap.sh",
        )
        if rc != 0:
