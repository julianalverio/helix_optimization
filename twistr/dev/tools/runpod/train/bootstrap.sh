...
# dev/tools/runpod/train/launch.py. Repo + runtime/data/examples/ already uploaded.
#
...
# source tree so config-relative paths like `runtime/data/examples/...` resolve.
ln -sfn /data data
...
echo "==> running training (runtime/configs/ml.yaml)"
python -m twistr.pipeline.training.train --config runtime/configs/ml.yaml
TRAIN_EXIT=$?
