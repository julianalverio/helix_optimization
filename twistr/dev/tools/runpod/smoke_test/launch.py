...
submodules and the data subset under dev/tools/runpod/smoke_test/subset/) to the
pod, runs `python -m twistr.pipeline.training.train` for max_steps=2, streams logs
...
Prereqs (the script checks all of these and bails fast if anything's missing):
  ...
  - dev/tools/runpod/smoke_test/subset/ (run make_subset.py first)
...
def main() -> None:
    ...
    try:
        ...
        rc = _ssh_exec_streaming(
            ...
            remote_cmd="bash /workspace/twistr/dev/tools/runpod/smoke_test/bootstrap.sh",
        )
        if rc != 0:
