...
Two-phase usage:
  Smoke (2 designs/face):      python -m twistr.dev.tools.runpod.boltzgen.launch_boltzgen --num-designs 2
  Production (50 designs/face): python -m twistr.dev.tools.runpod.boltzgen.launch_boltzgen --num-designs 50
"""
...
def _build_tarball(dest: Path, face: str) -> None:
    """Pack dev/tools/runpod/boltzgen + the renumbered target CIF + the
    ...
    with tarfile.open(dest, "w:gz") as tar:
        # scripts
        scripts = REPO_ROOT / "dev/tools/runpod/boltzgen"
        tar.add(scripts, arcname="dev/tools/runpod/boltzgen",
                filter=lambda ti: None
                if any(p in ("__pycache__", ".DS_Store") for p in ti.name.split("/"))
                or ti.name.endswith(".pyc")
                else ti)
        # tools/__init__.py — needed for `python -m twistr.dev.tools.runpod.boltzgen.run_boltzgen`
        ...
        # per-face spec — pod sees it under runtime/outputs/boltzgen_specs/<face>.yaml inside REPO,
...
def _send_tarball(host: str, port: int, tarball: Path, face: str,
    ...
    extract = subprocess.run(
        ["ssh", *SSH_OPTS, "-i", str(RUNPOD_SSH_KEY), "-p", str(port),
         ...
         "&& cp /workspace/twistr/runtime/outputs/boltzgen_specs/*.yaml /workspace/boltzgen_specs/"],
        capture_output=True, text=True,
    )
...
tmux new-session -d -s boltzgen \
    "BOLTZGEN_NUM_DESIGNS={num_designs} bash /workspace/twistr/dev/tools/runpod/boltzgen/boltzgen_bootstrap.sh > {log_path} 2>&1; echo \\$? > {sentinel}"
...
def _fetch_outputs(host: str, port: int, local_root: Path, face: str) -> None:
    ...
    subprocess.run(
        ["scp", *SSH_OPTS, "-r", "-i", str(RUNPOD_SSH_KEY), "-P", str(port),
         f"root@{host}:/workspace/boltzgen_runtime/outputs/.", str(local_root)],
        capture_output=True, text=True,
    )
    subprocess.run(
    ...
    for face in args.faces:
        if not (SPEC_DIR_LOCAL / f"{face}.yaml").is_file():
            sys.exit(f"missing spec: {SPEC_DIR_LOCAL / f'{face}.yaml'} — "
                     f"run `python -m twistr.dev.tools.runpod.boltzgen.build_specs ...` first")
