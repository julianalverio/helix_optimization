...
Reads `runtime/configs/pxdesign.yaml`, allocates a pod from `cfg.gpu_preferences`
(first available wins), ships the working tree (PXDesign submodule and
the `.cache/pxdesign_msa/` cache included), runs
`dev/tools/runpod/pxdesign/bootstrap.sh`, fetches the high-value outputs
...
DEFAULT_CONFIG = REPO_ROOT / "runtime/configs/pxdesign.yaml"
...
#   - `runtime/data/pdb/`: target PDB/CIF files the wrapper reads to compute the
...
DATA_KEEP_PREFIX = "runtime/data/pdb"
...
def _check_msa_cache(cfg: PXDesignConfig) -> None:
    ...
    if missing:
        ...
        sys.exit(
            f"missing MSA(s):\n{details}\n"
            "Build them first:\n"
            f"  python -m twistr.dev.tools.runpod.pxdesign.build_msas {DEFAULT_CONFIG.name}"
        )
...
def _tar_filter(tar_info: tarfile.TarInfo) -> tarfile.TarInfo | None:
    ...
    if parts[0] == ".cache":
        ...
        return None
    if parts[0] == "data":
        # Keep directory entries up to runtime/data/pdb/, plus any FILE directly in
        # runtime/data/pdb/ (i.e., runtime/data/pdb/<file>). We deliberately exclude
        # subdirectories under runtime/data/pdb/ — twistr's curation pipeline mirrors
        # the entire RCSB PDB there (~46 GB of <2char>/<id>.cif.gz files)
        # which we never need shipped to PXDesign pods.
        if name == "data" or name == "runtime/data/pdb":
            return tar_info
...
def _ssh_run_detached(host: str, port: int, remote_cmd: str, label: str = "",
    ...
    Mirrors the pattern in dev/tools/runpod/train/launch.py:223-249. Returns
...
def _fetch_results(host: str, port: int, cfg: PXDesignConfig) -> None:
    """Pull the entire runtime/outputs/design_runtime/outputs/<subdir>/ from the pod. Captures
    ...
    remote_root = f"/workspace/twistr/runtime/outputs/design_runtime/outputs/{cfg.output_subdir}"
...
def _ship_per_pod_config(host: str, port: int, cfg: PXDesignConfig, label: str) -> None:
    ...
    as /workspace/twistr/runtime/configs/pxdesign.yaml. Bootstrap reads from that
    ...
    try:
        scp = subprocess.run(
            ["scp", *SSH_OPTS, "-i", str(RUNPOD_SSH_KEY), "-P", str(port),
             str(local_cfg), f"root@{host}:/workspace/twistr/runtime/configs/pxdesign.yaml"],
            capture_output=True, text=True,
        )
        if scp.returncode != 0:
...
def _run_one_pod(cfg: PXDesignConfig, tarball: Path) -> bool:
    ...
    try:
        ...
        # Override the master runtime/configs/pxdesign.yaml with this pod's sub_cfg
        # so bootstrap.sh's hardcoded `--config runtime/configs/pxdesign.yaml` reads
        # the per-pod values (notably the suffixed output_subdir).
        _ship_per_pod_config(host, port, cfg, label)
        rc = _ssh_run_detached(
            host, port,
            "bash /workspace/twistr/dev/tools/runpod/pxdesign/bootstrap.sh",
            label=label,
        )
        if rc != 0:
