...
from twistr.ml.features.builder import build_features
from twistr.ml.models.architecture import (
    TorsionHead,
    HelixDesignModel,
    InputEmbedder,
    InteractionMatrixHead,
)
from twistr.ml.models.output_head import FrameOutputHead
from twistr.ml.models.pairformer import (
    PairformerBlock,
    PairformerStack,
    TriangleAttention,
...
def _real_batch():
    from twistr.ml.datasets.datamodule import pad_collate
    from twistr.ml.datasets.example_dataset import ExamplesDataset
...
def test_frame_head_starts_at_identity_and_passes_through_conditioning():
    ...
    from twistr.ml.models.rotation import rotation_from_6d
...
def test_lightning_module_training_step_runs():
    ...
    assert set(losses) == {
        ...
        "backbone_continuity", "dunbrack",
        "coord_mse_antigen_backbone", "coord_mse_annealed",
    }
    for name, value in losses.items():
        assert torch.isfinite(value), f"{name} loss is not finite: {value}"


_SECOND_NPZ = Path("data/module3/examples/mt/4mt8_1_6.npz")
...
def test_padding_does_not_change_real_residue_outputs():
    ...
    from twistr.ml.datasets.datamodule import _PAD_FILL, pad_collate
    from twistr.ml.datasets.example_dataset import ExamplesDataset
