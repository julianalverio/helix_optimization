...
Usage:
  python -m tools.runpod_pxdesign.launch_refold \\
      --inputs-dir refold_inputs \\
...
from tools.runpod_pxdesign.config import PXDesignConfig
...
REFOLD_BOOTSTRAP = REPO_ROOT / "tools/runpod_pxdesign/refold_bootstrap.sh"

# Lean tarball: only what the pod needs to run `protenix pred`.
#   tools/runpod_pxdesign/{run_protenix_refold.py, refold_bootstrap.sh, ...}
#   tools/__init__.py, twistr/__init__.py (for python -m tools.runpod_pxdesign)
#   .cache/pxdesign_msa/<key>/ — the cached target MSA (referenced in JSONs)
#   data/pdb/<target> — the target file (only needed for sanity / reference)
#
# Per-pod input JSON shards are scp'd separately after extraction.
_TARBALL_INCLUDE = [
    "tools/runpod_pxdesign",
]
...
tmux new-session -d -s refold "bash /workspace/twistr/tools/runpod_pxdesign/refold_bootstrap.sh > {log_path} 2>&1; echo \\$? > {sentinel}"
...
def _fetch_outputs(host: str, port: int, local_root: Path, label: str) -> None:
    ...
    r = subprocess.run(
        ["scp", *SSH_OPTS, "-r", "-i", str(RUNPOD_SSH_KEY), "-P", str(port),
         f"root@{host}:/workspace/refold_outputs/.", str(local_root)],
        capture_output=True, text=True,
    )
    if r.returncode != 0:
...
def main() -> None:
    ...
    cfg = PXDesignConfig(
        binder_length=80,
        target=__import__("tools.runpod_pxdesign.config", fromlist=["Target"]).Target(
